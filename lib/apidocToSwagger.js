...
function toSwagger(apidocJson, projectJson) {
    console.log('this is dulianqiang fork!');
...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
    ...
    pathParams = _.filter(pathParams, function(param) {
        var hasKey = pathKeys && pathKeys.indexOf(param.name) !== -1;
...
function createFieldArrayDefinitions(fieldArray, definitions, topLevelRef, defaultObjectName) {
    ...
    if (!fieldArray || !_.isArray(fieldArray)) {
        return result;
    }
    ...
    for (var i = 0; i < fieldArray.length; i++) {
        ...
        if (nestedName.propertyName) {
            ...
            var typeIndex = type && type.indexOf("[]");
            if (typeIndex !== -1 && typeIndex === ( type && type.length - 2)) {
                prop.type = "array";
                prop.items = {
                    type: type.slice(0, type && type.length - 2)
                };
            }
            ...
            if (!parameter.optional) {
                ...
                if (arr && arr.indexOf(nestedName.propertyName) === -1) {
...
function createPathParameters(verbs, pathKeys) {
    ...
    var pathItemObject = [];
    if (verbs.parameter && verbs.parameter.fields && _.isArray(verbs.parameter.fields.Parameter)) {
